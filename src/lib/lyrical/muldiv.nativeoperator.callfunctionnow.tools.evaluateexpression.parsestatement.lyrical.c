...
if (!(r1 || r2)) {
	...
	// I will use an unsigned instruction.
	// Note that signed and unsigned
	// low multiplications are the same.
	if (*funcarg->typepushed.ptr == 'u') {
		
		if (*name == '*') immresult = (imm1 * imm2);
		else if (*name == '/') {
			
			if (!imm2) throwerror("division by 0");
		...
		if (*name == '*') immresult = ((s64)imm1 * (s64)imm2);
		else if (*name == '/') {
...
if (r1) {
	...
	if (r2) {
		...
		if (*funcarg->typepushed.ptr == 'u') {
			...
			if (*name == '*') mul(rresult, r1, r2);
			else if (*name == '/') divu(rresult, r1, r2);
			else modu(rresult, r1, r2);
			
		} else {
			
			if (*name == '*') mul(rresult, r1, r2);
			else if (*name == '/') div(rresult, r1, r2);
			else mod(rresult, r1, r2);
		}
		...
		if (*funcarg->typepushed.ptr == 'u') {
			...
			if (*name == '*') muli(rresult, r1, imm2);
			else if (*name == '/') divui(rresult, r1, imm2);
			else modui(rresult, r1, imm2);
			
		} else {
			
			if (*name == '*') muli(rresult, r1, imm2);
			else if (*name == '/') divi(rresult, r1, imm2);
			else modi(rresult, r1, imm2);
		}
	}
	...
	if (*funcarg->typepushed.ptr == 'u') {
		...
		if (*name == '*') muli(rresult, r2, imm1);
		else if (*name == '/') divui2(rresult, r2, imm1);
		else modui2(rresult, r2, imm1);
		
	} else {
		
		if (*name == '*') muli(rresult, r2, imm1);
		else if (*name == '/') divi2(rresult, r2, imm1);
		else modi2(rresult, r2, imm1);
	}
}
