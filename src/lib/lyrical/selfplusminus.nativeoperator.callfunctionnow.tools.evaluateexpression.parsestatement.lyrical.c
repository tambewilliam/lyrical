...
if (r2) {
	...
	if (funcarg->typepushed.ptr[stringmmsz(funcarg->typepushed)-1] == '*' ||
		...
		// I allocate a new register which will contain
		// the computed value with the stride.
		...
		muli(r, r2, stride(funcarg->typepushed));
	...
	if (*name == '+') addi(r1, r1, imm2);
	else addi(r1, r1, -imm2);
}
