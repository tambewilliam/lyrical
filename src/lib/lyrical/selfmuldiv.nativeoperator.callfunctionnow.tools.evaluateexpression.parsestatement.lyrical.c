...
if (isvarreadonly(resultvar)) {
	...
	else if (*name == '/') throwerror("incorrect left argument to the native operator /=");
...
// the use of registers. So I make use of
// any loaded registers before
// they get discarded.
...
if (r2) {
	...
	if (*funcarg->typepushed.ptr == 'u') {
		...
		if (*name == '*') mul(r1, r1, r2);
		else if (*name == '/') divu(r1, r1, r2);
		else modu(r1, r1, r2);
		
	} else {
		
		if (*name == '*') mul(r1, r1, r2);
		else if (*name == '/') div(r1, r1, r2);
		else mod(r1, r1, r2);
	}
	...
	if (*funcarg->typepushed.ptr == 'u') {
		...
		if (*name == '*') muli(r1, r1, imm2);
		else if (*name == '/') divui(r1, r1, imm2);
		else modui(r1, r1, imm2);
		
	} else {
		
		if (*name == '*') muli(r1, r1, imm2);
		else if (*name == '/') divi(r1, r1, imm2);
		else modi(r1, r1, imm2);
	}
}
