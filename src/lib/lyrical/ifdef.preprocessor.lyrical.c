...
if (m) {
	...
	if (!*curpos) {
		...
		// it could have been `ifdef or `elifdef.
	...
	while (!checkfordirective("endif")) {
		// skipconditionalblock() will set curpos right before
		// either of the preprocessor directive `else,
		// `elifdef, `elifndef, `endif; it will also return
