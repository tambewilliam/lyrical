...
lyricaldbg32getlineresult lyricaldbg32getline (u8* dbgfilepath, u32 boff) {
	
	lyricaldbg32getlineresult result = {
		.ltxt = 0,
		.path = 0,
		.lnum = 0
	};
	...
	u32 dbginfosection1sz;
	
	// Read the size in bytes of section1 debug information.
	if (read(fid, &dbginfosection1sz, sizeof(u32)) != sizeof(u32)) {
	...
	u32 dbginfosection2offset = dbginfosection1sz + sizeof(u32);
	...
	do {
		...
		if (boff < dbginfosection1entry.boff) {
			...
			u32 filepathoffset = dbginfosection2offset + sizeof(u32) + dbginfosection1entry.path;
